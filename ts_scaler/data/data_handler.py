...
class DataHandler:
    ...
    def fetch_nyiso_data(
        self, local_dir: str = "./data", start_date: str = None, end_date: str = None
    ) -> pd.DataFrame:
        """Download NYISO data to the specified local directory for a date range."""
        if not start_date or not end_date:
            raise ValueError(
                "Both start_date and end_date must be provided in YYYYMMDD format."
            )
        ...
        while current_date_dt <= end_date_dt:
            ...
            if os.path.exists(local_file_path):
                self.logger.info(
                    f"Data for date {current_date_str} already exists locally. Reading the data."
                )
                all_data_frames.append(pd.read_csv(local_file_path))
            else:
                self.logger.info(
                    f"Data for date {current_date_str} not found locally. Downloading from S3."
                )
                self.s3_handler.download_files(
                    os.path.join(folder, target_file), local_dir
                )
                if os.path.exists(local_file_path):
                    self.logger.info(
                        f"Successfully downloaded data for date {current_date_str}. Reading the data."
                    )
                    all_data_frames.append(pd.read_csv(local_file_path))
                else:
                    self.logger.error(
                        f"Failed to download data for date {current_date_str} from S3."
                    )
